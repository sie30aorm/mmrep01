...
def generate_report(prefix, raw_prefix, from_date, to_date):
    ...
    if True:
      queries_jira['TRABPROG / CREATED FROM DATE']={
        "file": "{}_open_from_date".format(prefix),
        "raw": "{}_open_from_date".format(raw_prefix),
        "query": 'project = TP AND created >= {} AND created <= {} ORDER BY createdDate DESC, resolution DESC'.format(from_date, to_date)}

    if False:
      queries_jira['TRABPROG / CLOSED FROM DATE']={
        "file": "{}_closed_from_date".format(prefix),
        "raw": "{}_closed_from_date".format(raw_prefix),
        "query": 'project = TP AND resolved >= {} AND resolved <= {} ORDER BY createdDate DESC, resolution DESC'.format(from_date, to_date)}

    if False:
      queries_jira['TRABPROG / STILL OPEN FROM DATE']={
        "file": "{}_currently_open".format(prefix),
        "raw": "{}_currently_open".format(raw_prefix),
        "query": 'project = TP AND status in (CREADA, "In Progress") ORDER BY createdDate DESC, resolution DESC'}
...
opts = getConfig()
now=datetime.now()
prefix='{}/TRAB_PROG_{}_{}{}'.format(
				opts['out_dir'],
				opts['pp_from_date'],
				opts['pp_to_date'],
				opts['pp_hour']
				)
raw_prefix='{}/TRAB_PROG_{}_{}{}'.format(
				opts['raw_dir'],
				opts['pp_from_date'],
				opts['pp_to_date'],
				opts['pp_hour']
